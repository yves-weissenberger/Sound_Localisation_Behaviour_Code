...
volCorr = 0.2 #sensible range is between 0.1 and 2
#snds = [pygame.sndarray.make_sound((gensin(f,duration=dur)*volCorr).astype('int16')) for f in freqs]
#soundID = np.random.randint(nSounds)
snds = max16bit*np.random.random(size=sR*dur)/2
...
while True:
	...
	if (GPIO.event_detected(sndT)):
		...
		#snd = snds[soundID]
		#sndList.append([time.time()-start,str(soundID)])
                snd = pygame.sndarray.make_sound((snds*np.random.randint(30,150)/100).astype('int16'))
